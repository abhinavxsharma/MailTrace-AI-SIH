...
def _parse_from_normalized(norm: NormalizedEmail) -> ParsedEmail:
    ...
    # 1. Parse headers
    # NormalizedEmail headers is dict[str, str]
    header_list = parse_headers(norm.headers)

    # 2. Extract specific identity & routing headers
    from_raw = norm.headers.get("from") or norm.sender
    from_addr = parse_address(from_raw) if from_raw else None

    # Recipients (To, Cc, Bcc)
    to_raw = norm.headers.get("to", "")
    to_addrs = parse_addresses(to_raw) if to_raw else []

    cc_raw = norm.headers.get("cc", "")
    cc_addrs = parse_addresses(cc_raw) if cc_raw else []

    bcc_raw = norm.headers.get("bcc", "")
    bcc_addrs = parse_addresses(bcc_raw) if bcc_raw else []

    reply_to_raw = norm.headers.get("reply-to", "")
    reply_to_addrs = parse_addresses(reply_to_raw) if reply_to_raw else []

    sender_raw = norm.headers.get("sender", "")
    sender_addr = parse_address(sender_raw) if sender_raw else None

    return_path_raw = norm.headers.get("return-path", "")
    return_path_addr = parse_address(return_path_raw) if return_path_raw else None

    # 3. Message IDs & References
    msg_id_header = norm.headers.get("message-id")
    in_reply_to = norm.headers.get("in-reply-to")
    references = _parse_references(norm.headers.get("references"))

    # 4. Received chain
    received_hops: list[ReceivedHop] = []
    received_header_val = norm.headers.get("received")
    if received_header_val:
        # In NormalizedEmail, duplicate headers are joined with '; '
        # But a Received header internally has a semicolon before the date!
        # If multiple received hops were joined, we can split on '; from ' or '; by '
        # or parse as hops.
        # Check if multiple hops exist by looking for '; from ' or '; by '
    ...
    # 5. Extract URLs
    urls = extract_urls(norm.body_text, norm.body_html)

    # 6. Date
    date_val = norm.received_at or _parse_date(norm.headers.get("date"))

    return ParsedEmail(
        message_id=norm.provider_message_id,
        ...
        mime_type=norm.headers.get("content-type"),
        headers=header_list,
