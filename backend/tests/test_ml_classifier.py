...
class TestClassifier:
    ...
    def test_real_local_model_integration(self):
        ...
        """
        loader = ModelLoader()
        resolved_path = loader.get_resolved_path()
        has_real_weights = (
            resolved_path.is_dir()
            and (resolved_path / "config.json").is_file()
            and (
                (resolved_path / "model.safetensors").is_file()
                or (resolved_path / "pytorch_model.bin").is_file()
            )
        )
